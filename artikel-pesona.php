...
$input= $_GET['artikel'];
$target_url = $input;
$agenda = substr($input, 22, 6);
$keajaiban = substr($input, 22, 9);
$inspirasi = substr($input, 22, 9);
$destinasi = substr($input, 22, 9);
$curl = curl_init(); 
...
if($agenda == "agenda" || $inspirasi == "agenda" || $destinasi == "agenda" || $keajaiban == "agenda"){
foreach ($html->find(".big-title h3") as $archiveLink) {
    $judul = $archiveLink->plaintext;
	}
}elseif($agenda == "keajaiban" || $inspirasi == "keajaiban" || $destinasi == "keajaiban" || $keajaiban == "keajaiban"){
	foreach ($html->find(".ps50 h2") as $archiveLink) {
		$judul = $archiveLink->plaintext;
		}
}elseif($agenda == "inspirasi" || $inspirasi == "inspirasi" || $destinasi == "inspirasi" || $keajaiban == "inspirasi"){
	foreach ($html->find(".big-title h3") as $archiveLink) {
		$judul = $archiveLink->plaintext;
		}
}elseif($agenda == "destinasi" || $inspirasi == "destinasi" || $destinasi == "destinasi" || $keajaiban == "destinasi"){
	foreach ($html->find(".big-title h3") as $archiveLink) {
		$judul = $archiveLink->plaintext;
		}
}
				...
				<div class="col-lg-8 posts-list">
					<div class="single-post row">
						<div class="col-lg-12 col-md-12 col-sm-12">
							...
							<p><h5 class="mt-2">Sumber : <?php echo $input; ?></h5><br></p>
							<?php
							if($agenda == "agenda" || $inspirasi == "agenda" || $destinasi == "agenda" || $keajaiban == "agenda"){
								foreach ($html->find(".big-img-detail-box img") as $archiveLink) {
								$img = $archiveLink->src; }
							}elseif($agenda == "keajaiban" || $inspirasi == "keajaiban" || $destinasi == "keajaiban" || $keajaiban == "keajaiban"){
								foreach ($html->find(".big-img-detail img") as $archiveLink) {
								$img = $archiveLink->src; }
							}elseif($agenda == "inspirasi" || $inspirasi == "inspirasi" || $destinasi == "inspirasi" || $keajaiban == "inspirasi"){
								foreach ($html->find(".big-img-detail-box img") as $archiveLink) {
								$img = $archiveLink->src; }
							...
							<p class="excert text-justify">
								<?php
								foreach ($html->find(".detail-part p") as $archiveLink) {
